...
function extracts(messages: readonly CommonspaceMessage[], pattern: RegExp): string[] {
  return messages.flatMap(message => [...message.text.matchAll(pattern)].flatMap(match => match[1] ?? []))
}
...
export function createThreadContext(channelMemory: CommonspaceChannelMemory, capturedAt: string): CommonspaceThreadContext {
  return {
    channelSnapshot: {
      ...
      status: channelMemory.status ?? (channelMemory.summary === '' ? 'empty' : 'current'),
      sourceMessageCount: channelMemory.sourceMessageCount ?? 0,
      estimatedTokens: channelMemory.estimatedTokens ?? 0,
      compactedThroughMessageId: channelMemory.compactedThroughMessageId ?? null,
      capturedAt,
    },
...
export function projectThreadMemory(state: CommonspaceState, threadId: string): CommonspaceThreadMemory {
  const thread = state.threads.find(candidate => candidate.id === threadId)
  if (thread === undefined) throw new Error('unknown thread')
  const messages = (state.messages[conversationKey({ kind: 'channel', id: thread.channelId })] ?? [])
    .filter(message => message.threadId === thread.id)
  return projectThreadMemoryFromMessages(messages)
}

export function projectThreadMemoryFromMessages(messages: readonly CommonspaceMessage[]): CommonspaceThreadMemory {
  const decisions = unique(extracts(messages, /\b(?:decision|decided)\s*:\s*([^\n]+)/gim), 20)
  const explicitQuestions = extracts(messages, /\b(?:open question|question)\s*:\s*([^?\n]*\?)/gim)
  const sentenceQuestions = messages.flatMap(message => message.text
    .split(/(?<=[.!?])\s+/)
  ...
  return {
    summary: messages
      ...
      .map(message => `${message.authorName}: ${message.text.replace(/\s+/g, ' ').slice(0, 500)}`)
      .join('\n')
      .slice(-8_000),
    decisions,
...
export function mergeThreadMemoryProjection(
  ...
  return {
    ...
    status: current.status === 'failed'
      ? 'failed'
      : current.compactedThroughMessageId === projection.compactedThroughMessageId ? current.status : 'stale',
  }
}
...
export function buildThreadContextCompactionPrompt(
  ...
  const source = (state.messages[conversationKey({ kind: 'channel', id: thread.channelId })] ?? [])
    .filter(message => message.threadId === thread.id)
