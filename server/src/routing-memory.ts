...
function routingFeedback(state: CommonspaceState, channelId: string): RoutingFeedback[] {
  return (state.messages[conversationKey({ kind: 'channel', id: channelId })] ?? []).flatMap((message) => {
    if (message.routing === undefined) return []
    const assignments = new Map(message.routing.assignments.map(assignment => [assignment.id, assignment]))
...
export function buildRoutingMemoryCompactionPrompt(state: CommonspaceState, channelId: string): RoutingMemorySource | null {
  ...
  const agentNames = new Map(state.agents.map(agent => [agent.id, agent.displayName]))
  ...
  for (const item of feedback.toReversed()) {
    const correction = {
      ...
      from: {
        ...
        projects: item.from.projectIds.flatMap(projectId => projectNames.get(projectId) ?? []),
      },
      to: {
        agent: agentNames.get(item.to.agentId) ?? item.to.agentId,
        subRequest: item.to.subRequest,
        projects: item.to.projectIds.flatMap(projectId => projectNames.get(projectId) ?? []),
      },
      createdAt: item.correction.createdAt,
    }
    const serialized = JSON.stringify(correction)
    if (bounded.length > 0 && characters + serialized.length > MAX_ROUTING_FEEDBACK_CHARS) break
...
export function parseRoutingMemoryCompaction(text: string): string {
  ...
  if (typeof summary !== 'string') throw new Error('routing memory summary is required')
