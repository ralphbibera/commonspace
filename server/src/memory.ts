...
export function projectChannelMemory(
	...
	for (const thread of threads) {
		...
		const replySummary = replies
			...
			.map(
				(message) =>
					`${message.authorName}: ${message.text.replace(/\s+/g, " ").slice(0, 220)}`,
			)
			.join(" | ");
		summaryLines.push(
			`- ${root.text.replace(/\s+/g, " ").slice(0, 260)}${replySummary === "" ? "" : ` → ${replySummary}`}`,
		);
	}
	...
	sourceMessages.sort(
		(left, right) =>
			...
			(transcriptOrder.get(left.id) ?? 0) -
				(transcriptOrder.get(right.id) ?? 0),
	);
	const decisions = unique(
		extracts(sourceMessages, /\b(?:decision|decided)\s*:\s*([^\n]+)/gim),
		20,
	);
	...
	const sentenceQuestions = sourceMessages.flatMap((message) =>
		message.text
			.split(/(?<=[.!?])\s+/)
