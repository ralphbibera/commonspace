...
function highlightRanges(title: string, detail: string, includedTerms: readonly string[]): CommonspaceSearchHighlight[] {
  const ranges: CommonspaceSearchHighlight[] = []
  for (const [field, value] of [['title', title], ['detail', detail]] as const) {
  ...
  return ranges.sort((left, right) => left.field.localeCompare(right.field) || left.start - right.start)
}

function traceText(entry: CommonspaceTraceEntry): { title: string; detail: string } | null {
  if (entry.type === 'reasoning') return { title: 'Reasoning trace', detail: entry.text }
  if (entry.type === 'plan') return { title: 'Plan trace', detail: entry.markdown ?? entry.steps.map(step => step.text).join(' · ') }
  if (entry.type === 'tool') return { title: entry.title, detail: [entry.toolName, entry.input, entry.output].filter(Boolean).join(' · ') }
  return null
}

function conversationLabel(conversation: ConversationRef, bootstrap: CommonspaceBootstrap): string {
...
function liveActivityCandidates(bootstrap: CommonspaceBootstrap): Candidate[] {
  const messagesById = new Map(Object.values(bootstrap.state.messages).flat().map(message => [message.id, message]))
  const results: Candidate[] = []
  for (const activity of bootstrap.liveActivities ?? []) {
    ...
    const target = {
      ...
      messageId: source?.parentMessageId ?? source?.id ?? activity.sourceMessageId,
    }
...
function agentCandidates(bootstrap: CommonspaceBootstrap): Candidate[] {
  return bootstrap.agents.map(agent => ({
    ...
    detail: [agent.description, agent.adapter, agent.model, agent.status].filter(Boolean).join(' · '),
    receipt: `Agent · ${agent.adapter} · ${agent.status}`,
    target: { kind: 'agent', agentId: agent.id },
  }))
}

async function fileCandidates(state: CommonspaceState, includedTerms: readonly string[], projectFilter?: string): Promise<Candidate[]> {
  ...
  for (const project of state.projects) {
    ...
    for (let rootIndex = 0; rootIndex < project.paths.length; rootIndex += 1) {
      let root: string
      try { root = await realpath(project.paths[rootIndex] as string) } catch { continue }
      const pending: Array<{ absolute: string; relative: string }> = [{ absolute: root, relative: '' }]
      while (pending.length > 0 && scanned < MAX_FILE_ENTRIES_SCANNED && results.length < MAX_FILE_RESULTS) {
        const directory = pending.shift()
        if (directory === undefined) break
        let entries
        try { entries = await readdir(directory.absolute, { withFileTypes: true }) } catch { continue }
        for (const entry of entries) {
          if (scanned >= MAX_FILE_ENTRIES_SCANNED || results.length >= MAX_FILE_RESULTS) break
          scanned += 1
          if (entry.name === '.git' || entry.isSymbolicLink()) continue
          const path = directory.relative === '' ? entry.name : `${directory.relative}/${entry.name}`
          if (entry.isDirectory()) {
            pending.push({ absolute: join(directory.absolute, entry.name), relative: path })
            continue
          }
          if (!entry.isFile()) continue
          const searchable = normalized(`${project.name} ${path}`)
          if (!includedTerms.every(term => searchable.includes(term))) continue
          results.push({
...
export async function searchCommonspace(bootstrap: CommonspaceBootstrap, request: CommonspaceSearchRequest): Promise<CommonspaceSearchResponse> {
  const query = request.query.trim().slice(0, 500)
  const includedTerms = terms(query)
  const allowedKinds = new Set<CommonspaceSearchKind>(request.kinds?.filter(kind => COMMONSPACE_SEARCH_KINDS.includes(kind)) ?? [])
  const hasKindFilter = allowedKinds.size > 0
  const limit = Math.max(1, Math.min(MAX_LIMIT, Math.trunc(request.limit ?? DEFAULT_LIMIT)))
  const all = [
  ...
  const matching = all.filter(candidate => {
    ...
    return { ...result, highlights: highlightRanges(result.title, result.detail, includedTerms) }
  }).sort((left, right) => {
    const leftTitle = includedTerms.some(term => normalized(left.title).includes(term)) ? 1 : 0
    const rightTitle = includedTerms.some(term => normalized(right.title).includes(term)) ? 1 : 0
  ...
  return {
    ...
    appliedFilters: { kinds: [...allowedKinds], projectId: request.projectId ?? null },
