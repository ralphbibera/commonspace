...
type DesktopNotifier = (notification: CommonspaceDesktopNotification) => Promise<void>
...
function category(item: CommonspaceInboxItem): CommonspaceDesktopNotification['category'] {
...
function categoryEnabled(value: CommonspaceDesktopNotification['category'], settings: CommonspaceNotificationSettings): boolean {
...
function title(item: CommonspaceInboxItem): string {
  ...
  if (item.kind === 'mention') return `${item.actorName} mentioned you${location}`
  if (item.kind === 'permission-request') return `${item.actorName} needs permission${location}`
  if (item.kind === 'failure') return `${item.actorName} failed${location}`
  if (item.kind === 'timeout') return `${item.actorName} timed out${location}`
  if (item.kind === 'input-request') return `${item.actorName} needs input${location}`
...
export function desktopNotificationForItem(
  ...
  if (!settings.enabled || item.muted || !categoryEnabled(notificationCategory, settings)) return null
  const url = new URL('/', clientUrl)
  url.searchParams.set('conversation', item.conversation.kind)
  url.searchParams.set('conversationId', item.conversation.id)
  if (item.threadId !== undefined) url.searchParams.set('threadId', item.threadId)
...
export function createDesktopNotifier(): DesktopNotifier {
  ...
  return async notification => {
    if (center === undefined) {
      const loaded = moduleRequire('node-notifier') as { NotificationCenter: NotificationCenterConstructor }
    ...
    await new Promise<void>((resolve, reject) => {
      center?.notify({
        ...
        wait: false,
        timeout: 10,
      }, error => { if (error === null || error === undefined) resolve(); else reject(error) })
    })
  }
}
