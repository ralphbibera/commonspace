...
function isReadyMessage(value: unknown): boolean {
  return typeof value === 'object' && value !== null &&
    (value as { type?: unknown }).type === DEVELOPMENT_READY_MESSAGE
}
...
export function startDevelopmentSupervisor(options: StartDevelopmentSupervisorOptions): DevelopmentSupervisor {
  ...
  const startChild = (): void => {
    ...
    started.once('exit', (code, signal) => {
      ...
      if (expectedRestart) {
        ...
        logger.info('[commonspace:dev] Starting the updated server generation.')
        startChild()
        return
      }
      const outcome = code === null ? signal ?? 'unknown signal' : `code ${String(code)}`
      logger.error(`[commonspace:dev] Server exited with ${outcome}; waiting for a source change to restart.`)
    })
  }
  ...
  const dispatchRestart = (): void => {
    ...
    logger.info(`[commonspace:dev] ${reason ?? 'Source changed'}; keeping the current server alive until active agents finish.`)
    child.send({ type: DEVELOPMENT_RESTART_MESSAGE }, (error) => {
      if (error === null) return
      logger.error(`[commonspace:dev] Could not request a graceful restart: ${error.message}`)
    })
  }
  ...
  try {
    for (const path of options.watchPaths) {
      ...
      watcher.on('error', (error) => {
        logger.error(`[commonspace:dev] Watcher failed for ${path}: ${error.message}`)
      })
  ...
  return {
    ...
    async close() {
      closeOperation ??= (async () => {
        ...
        if (activeChild === undefined || activeChild.exitCode !== null || activeChild.signalCode !== null) return
        let exited = false
