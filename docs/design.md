---
version: alpha
name: Commonspace-design
description: Desktop workspace UI contract for a focused local-first conversation product. Layout and interaction stay stable while the semantic theme can be swapped independently.
...
- **Theme:** Colors come from the semantic CSS theme in `ui/src/index.css`. Do not hardcode a second palette into components.
- **Theme swapping:** Run `pnpm dlx shadcn@latest add "<tweakcn-theme-url>" --cwd ui --yes`. Layout, borders, components, and interaction behavior stay unchanged. Compatibility aliases bridge Commonspace-only surfaces, status colors, radii, controls, shadows, and motion to standard theme variables.
