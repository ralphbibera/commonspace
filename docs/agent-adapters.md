...
Use this guide to add a supported native harness to Commonspace. Start from the [adapter proposal template](templates/agent-adapter.md), then use [Claude Code](../server/src/adapters/claude-code.ts) as the smallest complete implementation.
...
The initial general-purpose baseline is Codex, Claude Code, Gemini CLI, and OpenCode; Hermes remains supported. Pi coding agent is a follow-up: its adapter must pass the same scoped MCP and native-session checks before registration. See the [support matrix](support-matrix.md#agent-runtimes) for version limits.
...
docs/templates/agent-adapter.md        Proposal and verification record
```
...
5. **Wire private configuration.** Add typed path overrides in `AgentAdapterConfig`, map environment variables in `server/src/index.ts`, and document defaults in [Operations](operations.md#runtime-configuration). Add every configured private path to the adapter's redaction list. Do not expose command configuration in browser contracts.
...
8. **Synchronize documentation.** Update the README, support matrix, product specification, architecture, development commands, acceptance coverage, roadmap, and changelog. Keep historical verification snapshots dated; add new evidence separately.
...
[Gemini CLI](https://geminicli.com/docs/cli/acp-mode/) supplies ACP directly through `gemini --acp`. Use version `0.43.0`; Commonspace currently accepts stable versions `>=0.39.1` and `<0.44.0`. Discovery and each process launch recheck the version. Native integration tests passed restart/resume with `0.39.1`, `0.41.2`, and `0.43.0`; tested releases from `0.44.1` through `0.58.0` lost prior context or failed `session/load`. The upstream [session-initialization investigation](https://github.com/google-gemini/gemini-cli/pull/28767) describes the ACP history-reset path; that proposed fix was closed without merging when checked on 2026-09-05. Expand the range only after the real runtime fixture passes. The normal mode is `default`; Full access requests `yolo`. Model selection uses the advertised native model control. Reasoning settings remain native defaults.
...
Normal checks use synthetic data and need no provider credentials:
...
The organization of shared metadata, runtime implementation, and authoring guidance is informed by [Paperclip's adapter guide](https://github.com/paperclipai/paperclip/blob/master/docs/adapters/creating-an-adapter.md). Commonspace keeps its conversation and ACP ownership rules above.

### Runtime verification without an account
...
Verified runtimes: `@agentclientprotocol/claude-agent-acp` 0.75.0 with bundled Claude Code 2.1.257, `@google/gemini-cli` 0.43.0, and `opencode-ai` 1.18.29. This establishes runtime integration without proving remote model quality, account access, or compatibility with every separately installed CLI. The earlier account-backed attempt with local Claude Code 2.1.104 timed out, as did standalone CLI requests; that installation can be checked independently with the opt-in commands above.
