...
This audit compares the intended behavior in [Product direction](product-direction.md) and [Product model](product.md) with executable contracts, server behavior, persistence, API routes, UI use, and tests. It deliberately separates product behavior from later UI/UX work.
...
| Zero/one/many Project references | Backend-ready | Messages, threads, search, MCP context, run attribution, and multi-root launches now support multiple references. The UI still uses the first reference as its compatibility selection. |
...
| Editable and compactable Channel context | Backend-ready | Read/update/manual-compact APIs, preserved user edits, stale/current state, and automatic token-pressure compaction now exist. Inspector/editor/compact controls remain UI work. |
...
1. Complete the inference layer: agent-specific sub-requests, Project-reference inference, reroute/correction, and compacted routing feedback.
2. Complete durable conversation semantics: thread context snapshots, message edit branches, deletion markers, and pins.
