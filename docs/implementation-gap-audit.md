...
| BYOA Agent identity | Working | Every new Agent is selected through explicit Hermes or Codex native-profile discovery. Commonspace-defined Codex personas and the managed `add-agent` mutation are removed; legacy persisted identities remain loadable so existing conversation history is not destroyed. |
