...
4. Agent profile/configuration center with inheritance and readiness diagnostics.
...
| Runtime settings                    | Implemented          | Global and per-channel model/reasoning settings, max agents per turn, memory window, runtime type/model/status, and on-demand discovery are visible. Agent profiles now read native tools, MCP integrations, skills, and connected services, and can save supported native model/reasoning settings with verification.                                        | `ui/src/CommonspaceSidebar.tsx:811`, `server/src/agent-configuration.ts:1`           |
