...
Research baseline 2026-08-28 against the current working tree and primary sources for Visual Studio Code, Block Buzz, Slack, ClickUp, and Paperclip AI. The two open-source comparators were also inspected at their current default branches. The current visual direction was updated 2026-09-03 in [UI direction](ui-direction.md).
...
The deeper comparison changes the emphasis. Commonspace now implements much of the first report's read-only trust layer—Project Files, Git Changes/diffs, image attachments, an actual-replies Inbox, richer Markdown, and native harness configuration/capability inspection. The next gaps are the surfaces that let one owner supervise several durable agent sessions without turning Commonspace into a ticketing product:
...
The supplied Apple workspace HTML is the current visual reference for desktop structure, not a source of product behavior or color settings. The implementation should port its layout, spacing, borders, row hierarchy, and pane composition while preserving Commonspace's semantic theme tokens.

The current acceptance surface is the desktop web application. Commonspace starts in Light mode and offers Light, Dark, and System choices. Targeted messages keep a neutral highlight card without an orange rail or tint. Routing metadata remains service-side for dispatch, reply binding, diagnostics, and correction; the conversation currently shows only pending and failed routing state, with resolved receipts and inline correction controls deferred.

See [UI direction](ui-direction.md) for the maintained visual rules and verification boundary.
...
| Agent DMs                           | Implemented          | Persistent one-to-one human/agent conversations with searchable agent picker. `/new` rotates the native session while preserving a visible transcript boundary.                                                                                                                                                                                               | `docs/product.md:26`                     |
...
| Threads                             | Implemented          | Each Channel root is a bounded unit of work with one exact native session; replies resume it. Active agents and queued/running/error state are visible.                                                                                                                                                                                                       | `ui/src/CommonspaceConversation.tsx:173` |
| Mentions and routing                | Implemented          | `@agent`, `@@project`, and `#channel` autocomplete exists. Explicit mentions remain authoritative; unmentioned messages persist inspectable per-harness assignments with bounded sub-requests and scoped Projects, using the visible max-agents limit. Resolved routing details remain at the service boundary for now.                                                                                                         | `ui/src/CommonspaceConversation.tsx`, `server/src/service.ts`                        |
...
| Agent reply Markdown                | Partial              | Agent messages use Streamdown in static mode with headings, emphasis, code, lists, tables, links, copy controls, safe external-image handling, and asynchronous placeholder behavior. User messages remain plain text; no rich composer or send-time preview exists.                                                                                          | `ui/src/MessageMarkdown.tsx:4`          |
...
| Files                               | Implemented          | A read-only Project browser traverses configured roots, keeps path handling server-side, and previews guarded text, image, and video files. It does not edit files, expose symbols, or attach a repository reference to a prompt.                                                                                                                             | `ui/src/CommonspaceProjectFiles.tsx:18`    |
| Git changes and diffs               | Partial              | The Project Changes view shows branch/status counts and unified per-file diffs. It is not yet attributed to a particular run, does not distinguish pre-existing changes, and lacks side-by-side/inline review, checkpoints, or integrate/discard actions.                                                                                                     | `ui/src/CommonspaceProjectChanges.tsx:40`  |
...
| Agent profile/catalog        | Known harness identities only              | Custom agents/skills            | Persona/identity                   | Agents & tools                      | AI Hub is strongest               | Agent/org/adapters            | Keep one truthful identity per supported harness; expose capability-dependent signals in conversation/session surfaces, not custom profile management. |
