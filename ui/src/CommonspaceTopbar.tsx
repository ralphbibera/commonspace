	...
	return (
		<header className="relative z-30 grid min-h-[52px] grid-cols-[240px_minmax(260px,640px)_240px] items-center justify-between gap-[18px] bg-sidebar-deep px-3.5 py-1 text-sidebar-foreground max-[780px]:grid-cols-[36px_minmax(0,1fr)] max-[780px]:gap-3 max-[780px]:px-3">
			<button
				...
				onClick={onOpenHome}
				className="flex min-h-11 w-fit items-center gap-2.5 rounded-sm border-0 bg-transparent px-0 text-left text-sidebar-foreground focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sidebar-foreground max-[780px]:px-2"
			>
			...
			<button
				...
				className="grid min-h-11 w-full grid-cols-[18px_minmax(0,1fr)_auto] items-center gap-2 rounded-md border border-sidebar-border bg-sidebar-accent px-2.5 text-left text-sidebar-foreground/80 hover:border-sidebar-foreground/40 hover:bg-sidebar-accent hover:text-sidebar-foreground focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sidebar-foreground"
			>
			...
			<span className="flex items-center justify-end gap-2 font-mono text-xs text-sidebar-foreground/70 max-[780px]:hidden">
				<i
