...
export function tagReferenceParts(text: string, bootstrap?: CommonspaceBootstrap): TagReferencePart[] {
  ...
  for (const match of text.matchAll(referencePattern)) {
    ...
    if (start > cursor) pushPart({ text: text.slice(cursor, start), kind: 'text' })
    const kind: Exclude<TagKind, 'text'> = marker === '@@' ? 'project' : marker === '@' ? 'agent' : 'channel'
    const normalizedValue = value.toLocaleLowerCase()
    const known = bootstrap === undefined
      || (kind === 'agent' && (normalizedValue === 'all' || bootstrap.agents.some(agent => agentMentionName(agent) === normalizedValue)))
      || (kind === 'project' && bootstrap.state.projects.some(project => project.id.toLocaleLowerCase() === normalizedValue || projectTagName(project.name) === normalizedValue))
      || (kind === 'channel' && bootstrap.state.channels.some(channel => channel.id.toLocaleLowerCase() === normalizedValue || channel.name.toLocaleLowerCase() === normalizedValue))
    pushPart({ text: marker + value, kind: known ? kind : 'text' })
    cursor = start + marker.length + value.length
  }
  if (cursor < text.length) pushPart({ text: text.slice(cursor), kind: 'text' })
...
export function tagSuggestions(
  ...
  if (prefix === '@') {
    const agents = bootstrap.agents
      ...
      .filter(({ agent, tagName }) => tagName.startsWith(query) || agent.displayName.toLocaleLowerCase().startsWith(query))
      .map(({ agent, tagName }) => ({
        kind: 'agent' as const,
        id: agent.id,
        ...
        ...(channelAgentIds === undefined
          ? {}
          : { channelMembership: channelAgentIds.includes(agent.id) ? 'member' as const : 'outside' as const }),
      }))
      .sort((left, right) => left.channelMembership === right.channelMembership || left.channelMembership === undefined
        ? 0
        : left.channelMembership === 'member' ? -1 : 1)
    const all = 'all'.startsWith(query)
      ? [{
          ...
          label: 'All agents',
          token: '@all',
          ...(channelAgentIds === undefined ? {} : { channelMembership: 'member' as const }),
        }]
      : []
