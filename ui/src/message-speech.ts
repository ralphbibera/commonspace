...
export function prepareMessageForSpeech(markdown: string): string {
  const prose = markdown
    ...
    .replace(/!\[([^\]]*)\]\([^)]*\)/g, (_, alt: string) => alt.trim() === '' ? '' : `Image: ${alt}.`)
    .replace(/\[([^\]]+)\]\([^)]*\)/g, '$1')
    ...
    .map(line => {
      ...
      if (trimmed.includes('|') && /^\|.*\|$/.test(trimmed)) {
        return sentence(trimmed.split('|').map(cell => cell.trim()).filter(Boolean).join(', '))
      }
...
export function useMessageSpeech(scopeKey: string | null): MessageSpeechControls {
  ...
  const toggle = useCallback((messageId: string, markdown: string) => {
    if (!supported) return
