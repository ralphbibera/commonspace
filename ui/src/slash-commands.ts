...
export function slashCommandSuggestions(
	...
	return COMMONSPACE_SLASH_COMMANDS.filter(
		(command) =>
			supportsContext(command, context) &&
			[command.name, ...command.aliases].some((token) =>
				token.toLocaleLowerCase().startsWith(prefix),
			),
	);
}
...
export function resolveSlashCommand(
	...
	const command = COMMONSPACE_SLASH_COMMANDS.find(
		(candidate) =>
			supportsContext(candidate, context) &&
			[candidate.name, ...candidate.aliases].some(
				(name) => name.toLocaleLowerCase() === token,
			),
	);
	if (command === undefined) return null;
	return {
		command,
		args: separator === -1 ? "" : normalized.slice(separator).trim(),
	};
}
