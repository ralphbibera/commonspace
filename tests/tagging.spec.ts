...
describe("Commonspace tagging", () => {
		...
		const agents = [
			{
				...
				description:
					"Owns React UI, browser interactions, styling, and accessibility.",
			},
			{
				id: "backend",
				displayName: "Backend",
				description:
					"Authoritative backend engineering specialist for APIs, services, data, auth, jobs, integrations, migrations, testing, runtime correctness, and application observability.",
			},
			{
				id: "infrastructure",
				displayName: "Infrastructure",
				description:
					"Owns deployment, containers, runtime operations, and observability.",
			},
		];
	...
	it("gives the AI router inspectable local ranking evidence", () => {
		expect(
			rankChannelAgents(
				["frontend", "backend"],
				"Fix persisted message validation.",
				[
					{
						id: "frontend",
						displayName: "Frontend",
						description: "Owns React UI and styling.",
					},
					{
						id: "backend",
						displayName: "Backend",
	...
	it("falls back to one agent instead of broadcasting when no routing metadata is available", () => {
		expect(
			routeChannelAgents(["backend", "frontend"], "Please take a look.", [
				{ id: "backend", displayName: "Backend" },
				{ id: "frontend", displayName: "Frontend" },
			]),
		).toEqual(["backend"]);
	...
	it("routes unique workspace names without exposing native profile ids", () => {
		...
		expect(
			routeChannelAgents(
				["default", "backend"],
				"@agentops please check",
				agents,
			),
		).toEqual(["default"]);
		expect(
			mentionedAgents("@default does not address a hidden id", agents),
		).toEqual([]);
	});
