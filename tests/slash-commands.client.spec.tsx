...
describe("Commonspace composer commands", () => {
	it("opens a command menu from slash and shows local status without messaging the agent", () => {
		...
		expect(composer.getAttribute("aria-activedescendant")).toContain(
			`${listbox.id}-option-`,
		);
		...
		expect(
			screen.getByRole("status", { name: "Command result" }).textContent,
		).toContain("Review Bot");
		expect(
			screen.getByRole("status", { name: "Command result" }).textContent,
		).toContain("Codex");
		expect(send).not.toHaveBeenCalled();
	...
	it("stops the work associated with the latest message", async () => {
		...
		await waitFor(() => {
			expect(stopAgentRuns).toHaveBeenCalledWith("message-1");
		});
	...
	it("confirms before starting a fresh direct-message session", async () => {
		...
		expect(
			screen.getByRole("status", { name: "Command result" }).textContent,
		).toContain("keeps earlier messages visible");
	...
	it("does not forward an unknown slash command to the agent", () => {
		...
		expect(
			screen.getByRole("alert", { name: "Command result" }).textContent,
