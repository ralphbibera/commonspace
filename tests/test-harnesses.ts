import type { AgentAdapterKind, CommonspaceAgentProfile } from '@commonspace/shared'
import type { CommonspaceHostService } from '../server/src/service.ts'

export async function discoverTestHarnesses(adapter: AgentAdapterKind): Promise<CommonspaceAgentProfile[]> {
...
export async function addTestHarness(
  ...
  if (displayName !== undefined && displayName !== harness.displayName) {
    await service.mutate({ action: 'update-agent-profile', agentId: harness.id, displayName })
  }
  return (await service.bootstrap()).agents.find(candidate => candidate.id === harness.id)!
}
