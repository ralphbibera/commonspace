...
describe('general file attachments', () => {
  it('persists and delivers a safe file without exposing its host URI to the browser', async () => {
    ...
    const channel = (await service.mutate({ action: 'create-channel', name: 'files', agentIds: ['codex'] })).channels[0]!
    ...
    expect(file).toMatchObject({ id: expect.any(String), name: 'notes.txt', mimeType: 'text/plain', size: 18 })
    ...
    expect(await readFile(fileURLToPath(deliveredUri!), 'utf8')).toBe('verification notes')
    expect(JSON.stringify(await service.bootstrap())).not.toContain(deliveredUri)
    await expect(service.readFileAttachment(file!.id)).resolves.toMatchObject({ metadata: file, data: Buffer.from('verification notes') })
    await service.close()
    ...
    await expect(restarted.readFileAttachment(file!.id)).resolves.toMatchObject({ metadata: file, data: Buffer.from('verification notes') })
  ...
  it('refuses credential-bearing files before accepting a message', async () => {
    ...
    await expect(service.send({
      ...
      text: 'Inspect this.',
      files: [{ name: '.env', mimeType: 'text/plain', data: Buffer.from('TOKEN=secret').toString('base64') }],
    } as never)).rejects.toThrow('credential-bearing files cannot be attached')
