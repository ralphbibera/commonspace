...
describe("Commonspace Inbox state", () => {
	...
	it("persists follow, mute, and save-for-later independently", () => {
		...
		const saved = applyMutation(state, {
			action: "set-inbox-item-saved",
			messageId: "reply-1",
			saved: true,
		});
		const followed = applyMutation(saved, {
			action: "set-session-followed",
			sessionId: "request-1:backend",
			followed: true,
		});
		const muted = applyMutation(followed, {
		...
		expect(deriveCommonspaceInboxItems(muted)[0]).toMatchObject({
			saved: true,
			muted: true,
			unread: false,
		});
	...
	it("marks one agent reply read without clearing a different unread reply", () => {
		...
		expect(items.find((item) => item.id === "message:reply-2")?.unread).toBe(
			false,
		);
		expect(items.find((item) => item.id === "message:reply-1")?.unread).toBe(
			true,
		);
	...
	it("persists the read cursor when all Inbox activity is marked read", () => {
		...
		expect(
			deriveCommonspaceInboxItems(state).filter((item) => item.unread),
		).toHaveLength(1);
		...
		expect(
			deriveCommonspaceInboxItems(next).filter((item) => item.unread),
		).toHaveLength(0);
		expect(next.revision).toBe(1);
	});
});
