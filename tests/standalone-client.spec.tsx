...
describe("standalone Commonspace application", () => {
	it("opens on the source-aligned workspace home and keeps Inbox navigation available", () => {
		...
		fireEvent.click(
			mustExist(
				screen.getAllByRole("button", { name: "Open Commonspace home" })[0],
			),
		);
		expect(screen.getByRole("heading", { name: "Agent runs" })).toBeTruthy();
	});
	...
	it("mounts navigation and workspace home without host slots", () => {
		...
		const view = render(<CommonspaceApp store={store} />);
	...
	it("configures one AI routing model for the whole Commonspace", async () => {
		...
		expect(
			screen.getByRole("heading", { name: "Choose how the workspace thinks" }),
		).toBeTruthy();
		expect(
			screen
				.getByRole("button", {
					name: "Use OpenAI-compatible inference for routing",
				})
				.getAttribute("aria-pressed"),
		).toBe("true");
		expect(
			screen.getByLabelText("Routing API key").getAttribute("placeholder"),
		).toContain("Saved");
		fireEvent.change(screen.getByLabelText("Routing model"), {
			target: { value: "gpt-4.1-mini" },
		});
		fireEvent.change(screen.getByLabelText("Routing API key"), {
			target: { value: "new-secret" },
		});
		fireEvent.click(
			screen.getByRole("button", { name: "Save inference settings" }),
		);
	...
	it("requires an inference-backed routing engine", async () => {
		...
		expect(
			screen.queryByRole("button", { name: "Use local routing" }),
		).toBeNull();
		expect(
			screen.getByRole("group", { name: "Agent run defaults" }),
		).toBeTruthy();
		fireEvent.click(
			screen.getByRole("button", {
				name: "Use native agent inference for routing",
			}),
		);
		fireEvent.click(
			screen.getByRole("button", { name: "Use Backend agent for routing" }),
		);
		...
		await waitFor(() => {
			expect(updateRoutingConfiguration).toHaveBeenLastCalledWith({
				provider: "harness",
	...
	it("opens a search result at its channel root message instead of its thread", async () => {
							...
							results: [
								{
									...
									kind: "message",
									title: "AgentOps",
									detail: "Unique searchable reply",
									receipt: "#general · AgentOps · 2026-08-26T00:01:00.000Z",
									highlights: [],
									target: {
										kind: "conversation",
										conversation: { kind: "channel", id: "general" },
		...
		fireEvent.change(
			screen.getByRole("searchbox", { name: "Search Commonspace" }),
			{ target: { value: "Unique searchable reply" } },
		);
		fireEvent.click(
			await screen.findByRole("option", { name: "Open Message: AgentOps" }),
		);
	...
	it("boots into a normal DOM root with standalone styles", () => {
		...
		expect(
			document.querySelector('style[data-commonspace="standalone"]'),
		).toBeTruthy();
		expect(
			root.querySelector('main[aria-label="Workspace home"]'),
		).toBeTruthy();
		mounted.unmount();
		expect(
			document.querySelector('style[data-commonspace="standalone"]'),
