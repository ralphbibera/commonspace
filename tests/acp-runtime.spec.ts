...
describe('ACP agent process', () => {
  it('starts a session and sends only the new message as the prompt', async () => {
    ...
    try {
      ...
      expect(prompt?.params.prompt).toEqual([{ type: 'text', text: 'Only this new message.' }])
    } finally {
  ...
  it('returns a provider-neutral activity trace from ACP session updates', async () => {
    ...
    try {
      const result = await processClient.run({ cwd: root, message: 'Trace this turn.' })
      ...
      expect(result).toMatchObject({
        ...
        trace: {
          ...
          entries: [
            ...
            expect.objectContaining({ type: 'usage', usedTokens: 640, contextWindow: 128000 }),
          ],
        },
      })
    } finally {
  ...
  it('loads a persisted native session without leaking replayed history into the next reply', async () => {
    ...
    const processClient = new AcpAgentProcess({
      ...
      args: [fixturePath],
      cwd: root,
      env: { ...process.env, FAKE_ACP_LOG: logPath, FAKE_ACP_REPLAY_ON_LOAD: '1' },
    })
  ...
  it('terminates an agent that exceeds the ACP protocol frame limit', async () => {
    ...
    try {
      await expect(processClient.run({ cwd: root, message: 'Overflow.' }))
  ...
  it('cancels and rejects a response that exceeds the published output limit', async () => {
    ...
    const processClient = new AcpAgentProcess({
      ...
      args: [fixturePath],
      cwd: root,
      env: { ...process.env, FAKE_ACP_LOG: logPath, FAKE_ACP_LARGE_CHUNK: '2000' },
  ...
  it('safely rejects an unexpected ACP permission request', async () => {
    ...
    const processClient = new AcpAgentProcess({
      ...
      args: [fixturePath],
      cwd: root,
      env: { ...process.env, FAKE_ACP_LOG: logPath, FAKE_ACP_PERMISSION_REQUEST: '1' },
    })
    ...
    try {
      ...
      await vi.waitFor(async () => {
        const frames = (await readFile(logPath, 'utf8')).trim().split('\n').map(line => JSON.parse(line))
        expect(frames.find(frame => frame.id === 'permission-1' && frame.method === undefined)?.result)
  ...
  it('closes promptly while ACP initialization is still pending', async () => {
    const root = await mkdtemp(join(tmpdir(), 'commonspace-acp-close-pending-'))
    roots.push(root)
    const processClient = new AcpAgentProcess({
      command: process.execPath,
      args: [fixturePath],
    ...
    const running = processClient.run({ cwd: root, message: 'Never delivered.' })
    await delay(25)
  ...
  it('reloads an active native session when its session-scoped MCP binding changes', async () => {
    ...
    const firstMcp = [{ type: 'http' as const, name: 'commonspace', url: 'http://127.0.0.1:3100/api/mcp', headers: [{ name: 'Authorization', value: 'Bearer first' }] }]
    const secondMcp = [{ type: 'http' as const, name: 'commonspace', url: 'http://127.0.0.1:3100/api/mcp', headers: [{ name: 'Authorization', value: 'Bearer second' }] }]

    try {
      const first = await processClient.run({ cwd: root, message: 'First.', mcpServers: firstMcp })
      await processClient.run({ cwd: root, message: 'Second.', sessionId: first.sessionId, mcpServers: secondMcp })
      const frames = (await readFile(logPath, 'utf8')).trim().split('\n').map(line => JSON.parse(line))
      expect(frames.filter(frame => frame.method === 'session/load')).toHaveLength(1)
      expect(frames.find(frame => frame.method === 'session/load')?.params.mcpServers[0].headers[0].value)
        .toBe('Bearer second')
    } finally {
      await processClient.close()
    }
