...
function appStore(initialState: CommonspaceState, includeLiveActivity = true) {
	...
	const mutate = vi.fn(async (mutation: CommonspaceMutation) => {
		...
		if (mutation.action === "set-inbox-item-saved") {
			preferenceUpdate = {
				inboxSavedItemIds: mutation.saved
					? [...current.inboxSavedItemIds, mutation.messageId]
					: current.inboxSavedItemIds.filter((id) => id !== mutation.messageId),
			};
		} else if (mutation.action === "set-session-followed") {
			preferenceUpdate = {
				followedSessionIds: mutation.followed
					? [...current.followedSessionIds, mutation.sessionId]
					: current.followedSessionIds.filter(
							(id) => id !== mutation.sessionId,
						),
				mutedSessionIds: mutation.followed
					? current.mutedSessionIds.filter((id) => id !== mutation.sessionId)
					: current.mutedSessionIds,
			};
		} else if (mutation.action === "set-session-muted") {
			preferenceUpdate = {
				mutedSessionIds: mutation.muted
					? [...current.mutedSessionIds, mutation.sessionId]
					: current.mutedSessionIds.filter((id) => id !== mutation.sessionId),
				followedSessionIds: mutation.muted
					? current.followedSessionIds.filter((id) => id !== mutation.sessionId)
					: current.followedSessionIds,
			};
		} else if (
				...
				state: {
					...
					...(mutation.action === "mark-inbox-read"
						...
						: mutation.action === "mark-inbox-item-read"
							? {
									inboxReadMessageIds: [
										...snapshot.bootstrap.state.inboxReadMessageIds,
										mutation.messageId,
									],
								}
							: preferenceUpdate),
...
describe("Commonspace Inbox", () => {
	it("opens an exact notification deep link on first load", async () => {
		...
		window.history.replaceState(
			null,
			"",
			"/?conversation=channel&conversationId=general&threadId=thread-1&messageId=reply-thread",
		);
		const { store, selectConversation, selectThread } = appStore(
			state(),
			false,
		);

		render(<CommonspaceApp store={store} />);
		...
		expect(
			await screen.findByText("Thread result.", {}, { timeout: 3_000 }),
		).toBeTruthy();
		await waitFor(() => {
			expect(
				document.getElementById("commonspace-message-reply-thread"),
			).not.toBeNull();
	...
	it("shows the unread Inbox count on its channel", () => {
		...
		const channel = screen.getByRole("button", {
			name: "Open channel general, 1 unread",
		});
	...
	it("opens a real Threads index and filters unread conversations", () => {
		...
		fireEvent.click(
			screen.getByRole("button", { name: "Open Threads, 1 unread" }),
		);

		const threads = screen.getByRole("main", { name: "Threads" });
		expect(
			within(threads).getByRole("heading", { name: "Threads" }),
		).toBeTruthy();
		expect(within(threads).getByText("Please investigate this.")).toBeTruthy();
		expect(
			within(threads).getByRole("button", { name: "Mark all read" }),
		).toBeTruthy();
	...
	it("marks only the opened Inbox reply read", async () => {
		...
		fireEvent.click(
			screen.getByRole("button", { name: "Open Inbox, 2 unread" }),
		);
		fireEvent.click(
			screen.getByRole("button", {
				name: "Open completed from Backend in #general, unread",
			}),
		);
		...
		await waitFor(() => {
			expect(mutate).toHaveBeenCalledWith({
				...
				messageId: "reply-thread",
			});
			expect(mutate).not.toHaveBeenCalledWith({ action: "mark-inbox-read" });
		});
		expect(
			screen.getByRole("button", { name: "Open Inbox, 1 unread" }),
		).toBeTruthy();
		...
		expect(
			screen.getByRole("button", {
				name: "Open completed from Reviewer in Reviewer, unread",
			}),
		).toBeTruthy();
	...
	it("opens from navigation, filters unread replies, marks them read, and opens a thread", async () => {
		...
		const { store, mutate, selectConversation, selectThread } = appStore(
			state(),
		);
		render(<CommonspaceApp store={store} />);
		...
		expect(
			within(inbox).getByText(
				"Agent replies, requests, and native session outcomes",
			),
		).toBeTruthy();
		expect(
			within(inbox).getByRole("button", { name: "Mark all read" }),
		).toBeTruthy();
		...
		expect(
			await within(inbox).findByText("You’re all caught up."),
		).toBeTruthy();
		...
		fireEvent.click(
			within(inbox).getByRole("button", {
				name: "Open completed from Backend in #general",
			}),
		);
		...
		expect(
			document
				.getElementById("commonspace-message-root-1")
	...
	it("shows a meaningful empty state", () => {
		const { store } = appStore(
			state({ channels: [], threads: [], messages: {}, agents: [] }),
			false,
		);
		render(<CommonspaceApp store={store} />);
		...
		expect(
			screen.getByText("New agent activity will appear here."),
		).toBeTruthy();
	});
	...
	it("saves attention items and supervises running, attention, and completed sessions", async () => {
		...
		const initial = state({
			messages: {
				...state().messages,
				"dm:reviewer": [...mustExist(state().messages["dm:reviewer"]), failed],
			},
		});
		...
		fireEvent.click(
			mustExist(screen.getAllByRole("button", { name: "Save for later" })[0]),
		);
		await waitFor(() => {
			expect(mutate).toHaveBeenCalledWith({
				action: "set-inbox-item-saved",
				messageId: "failed-request",
				saved: true,
			});
		});
