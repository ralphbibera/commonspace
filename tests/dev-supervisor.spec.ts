...
import { startDevelopmentSupervisor, type DevelopmentSupervisor } from '../server/src/dev-supervisor.ts'

const fixturePath = join(dirname(fileURLToPath(import.meta.url)), 'fixtures', 'fake-development-server.mjs')
const roots: string[] = []
...
describe('development server supervisor', () => {
  it('coalesces edits while the old server generation drains before starting the next one', async () => {
    ...
    await vi.waitFor(async () => {
      expect((await logLines(logPath)).filter(line => line.startsWith('started:'))).toHaveLength(1)
    })
    await writeFile(watchedPath, 'first edit')
    await vi.waitFor(async () => {
      expect((await logLines(logPath)).some(line => line.startsWith('restart-requested:'))).toBe(true)
    })
    await writeFile(watchedPath, 'second edit while draining')

    await new Promise(resolve => setTimeout(resolve, 50))
    expect((await logLines(logPath)).filter(line => line.startsWith('started:'))).toHaveLength(1)
    expect((await logLines(logPath)).some(line => line.startsWith('forced:'))).toBe(false)

    await vi.waitFor(async () => {
      expect((await logLines(logPath)).filter(line => line.startsWith('started:'))).toHaveLength(2)
    }, { timeout: 1_500 })
