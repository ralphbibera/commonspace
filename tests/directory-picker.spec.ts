...
describe('project directory selection', () => {
  it('uses the repository root when a nested Git directory is selected', async () => {
    const repository = await mkdtemp(join(tmpdir(), 'commonspace-directory-picker-'))
    roots.push(repository)
    const nestedDirectory = join(repository, 'packages', 'ui')
    await mkdir(nestedDirectory, { recursive: true })
    await execFileAsync('git', ['init', '--quiet'], { cwd: repository })

    await expect(resolveProjectDirectory(nestedDirectory)).resolves.toBe(await realpath(repository))
  })
})
