...
async function fixture() {
	...
	const runAgent = vi.fn(
		async (): Promise<{ text: string }> => ({ text: "Done." }),
	);
...
describe("multi-project conversation context", () => {
	...
	it("delivers every tagged Project to a direct-message agent", async () => {
		const { service, runAgent, first, second, firstRoot, secondRoot } =
			await fixture();
	...
	it("changes Thread Project references only for the new turn and future defaults", async () => {
		...
		await service.whenIdle();
		expect(compatibilityReply.accepted.projectIds).toEqual([
			first.id,
			second.id,
		]);
		...
		expect(
			service.snapshot().threads.find((thread) => thread.id === root.thread?.id)
				?.projectIds,
		).toEqual([first.id]);
		...
		expect(
			service
				...
				.messages[`channel:${channel.id}`]?.find(
					(message) => message.id === root.accepted.id,
				)?.projectIds,
	...
	it("keeps an active turn scoped to Projects delivered before future Thread defaults change", async () => {
		...
		runAgent
			.mockImplementationOnce(async (input: AgentRunInput) => {
		...
		const reading = service.readContext(mustExist(firstScope));
	...
	it("removes only the deleted Project from multi-project references", async () => {
		...
		expect(
			service
				...
				.messages["dm:codex"]?.some(
					(message) =>
						message.projectIds?.includes(first.id) === true &&
						message.projectIds.includes(second.id),
				),
		).toBe(true);
		const beforeRemoval = service
			.snapshot()
			.messages["dm:codex"]?.find((message) => message.authorType === "agent");
		expect(
			beforeRemoval?.runAttribution?.roots.map((root) => root.projectId),
		).toEqual([first.id, second.id]);
		...
		expect(
			messages.every(
				(message) => message.projectIds?.includes(first.id) !== true,
			),
		).toBe(true);
		expect(
			messages
				.filter((message) => message.projectIds !== undefined)
				.every((message) => message.projectId === second.id),
		).toBe(true);
		const afterRemoval = messages.find(
			(message) => message.authorType === "agent",
		);
		expect(
			afterRemoval?.runAttribution?.roots.map((root) => root.projectId),
		).toEqual([second.id]);
	});
	...
	it("does not launch queued work with stale Project authority", async () => {
		...
		expect(messages.some((message) => message.authorType === "agent")).toBe(
			false,
		);
	});
	...
	it("removes attribution roots for Projects whose paths disappear before startup", async () => {
		...
		expect(restarted.snapshot().projects.map((project) => project.id)).toEqual([
			second.id,
		]);
		const reply = restarted
			.snapshot()
			.messages["dm:codex"]?.find((message) => message.authorType === "agent");
		expect(reply?.projectIds).toEqual([second.id]);
		expect(reply?.runAttribution?.roots.map((root) => root.projectId)).toEqual([
			second.id,
		]);
	});
});
