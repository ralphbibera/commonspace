...
describe("Commonspace direct messages", () => {
	it("shows when a direct-message agent is responding", () => {
		...
		expect(
			screen
				.getByRole("button", { name: "Frontend activity" })
				.getAttribute("aria-expanded"),
		).toBe("false");
		fireEvent.click(stop);
		expect(store.stopAgentRuns).toHaveBeenCalledWith("message-1", "frontend");
		expect(
			screen.getByRole("region", { name: "Queued follow-ups" }).textContent,
		).toContain("Then check the narrow layout.");
		fireEvent.click(
			screen.getByRole("button", { name: "Remove queued follow-up" }),
		);
		expect(store.removeFollowup).toHaveBeenCalledWith("message-2");

		fireEvent.change(
			screen.getByRole("textbox", { name: "Message Frontend" }),
			{ target: { value: "Use this direction instead." } },
		);
		expect(
		...
		fireEvent.submit(
			mustExist(
				screen
					.getByRole("textbox", { name: "Message Frontend" })
					.closest("form"),
			),
		);
	...
	it("starts a direct message from the Agents section with configuration beside each agent", () => {
		...
		const backend = screen.getByRole("button", {
			name: "Message agent Backend",
		});
		expect(backend.getAttribute("aria-pressed")).toBe("true");
		expect(
			screen.getByRole("button", { name: "Customize agent Backend" }),
		).toBeTruthy();
		expect(
			screen.getByRole("button", { name: "Customize agent Review Bot" }),
		).toBeTruthy();
		fireEvent.click(
			screen.getByRole("button", { name: "Message agent Review Bot" }),
		);
