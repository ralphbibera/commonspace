...
function renderAgentMessage(
			...
			state: {
				...
				projects:
					runAttribution === undefined
						? []
						: [{ id: "project-1", name: "App", paths: ["[host path]"] }],
				channels: [],
				threads: [],
	...
	return render(<CommonspaceConversation store={store} />);
}
...
describe("Commonspace message markdown", () => {
	it("keeps an accessible message placeholder visible while the Markdown renderer loads", () => {
		...
		expect(
			screen.getByRole("status", { name: "Formatting agent message" }),
		).toBeTruthy();
	});
	...
	it("renders structured assistant markdown instead of showing its source punctuation", async () => {
		...
		expect(
			container.querySelector('[data-selectable-text="true"] code')
				?.textContent,
		).toBe("inline()");
		expect(screen.getByRole("list").children).toHaveLength(2);
		expect(screen.getByRole("table")).toBeTruthy();
		expect(
			screen.getByRole("link", { name: "Hermes" }).getAttribute("href"),
		).toBe("https://hermes-agent.nousresearch.com/docs");
	...
	it("does not load agent-provided image URLs until the user chooses to open them", async () => {
		...
		const imageLink = await screen.findByRole("link", {
			name: "Open image: Tracking pixel",
		});
	...
	it("reads an agent message aloud as prose and exposes playback controls", () => {
		...
		expect(
			screen
				.getByRole("button", { name: "Read message aloud" })
	...
	it("reveals the harness-emitted reasoning, plan, tool calls, and usage for an agent reply", () => {
		...
		expect(
			screen.queryByRole("region", { name: "Writer activity trace" }),
		).toBeNull();
		const toggle = screen.getByRole("button", {
			name: "Show Codex activity for Writer",
		});
		...
		expect(trace.textContent).toContain(
			"Inspecting the workspace and choosing the smallest safe change.",
		);
		...
		expect(
			screen.getByRole("button", { name: "Hide Codex activity for Writer" }),
		).toBeTruthy();
	});
	...
	it("shows repository changes and pre-existing work bound to the agent reply", () => {
		...
		expect(
			within(evidence).getByRole("button", {
				name: "Open src/result.ts in editor",
			}),
		).toBeTruthy();
	});
});
