...
import {
	type CommonspaceDesktopNotification,
...
describe("desktop notification delivery", () => {
	it("delivers only new enabled Inbox events and does not replay them after restart", async () => {
		...
		const notify = vi.fn(
			async (notification: CommonspaceDesktopNotification) => {
		...
		expect(delivered[0]).toMatchObject({
			category: "reply",
			title: "Codex replied in Codex",
		});
		expect(
			new URL(mustExist(delivered[0]).url).searchParams.get("messageId"),
		).toBeTruthy();
		expect(
			deriveCommonspaceInboxItems(service.snapshot()).filter(
				(item) => item.kind === "completion",
			),
		).toHaveLength(2);
		await service.close();
	...
	it("delivers mentions, permission requests, and failures through their own categories", async () => {
		const root = await mkdtemp(
			join(tmpdir(), "commonspace-notification-categories-"),
		);
		roots.push(root);
		const delivered: CommonspaceDesktopNotification[] = [];
		...
		await service.whenIdle();
		expect(
			delivered.some((notification) => notification.category === "mention"),
		).toBe(true);
		...
		await service.whenIdle();
		expect(
			delivered.some((notification) => notification.category === "failure"),
		).toBe(true);
		...
		await vi.waitFor(() => {
			expect(
				delivered.some(
					(notification) => notification.category === "permission",
				),
			).toBe(true);
		});
		const permission = mustExist(
			service
				.snapshot()
				.permissions.find((candidate) => candidate.status === "pending"),
		);
	...
	it("keeps the durable result when native notification delivery fails", async () => {
		const root = await mkdtemp(
			join(tmpdir(), "commonspace-notification-failure-"),
		);
		roots.push(root);
		const warn = vi.fn();
		const service = new CommonspaceHostService(
			...
			{ root },
			{
				discoverAgents: discoverTestHarnesses,
				runAgent: async () => ({
					text: "Durable despite notification failure.",
				}),
				notify: async () => {
		...
		expect(
			service
				...
				.messages["dm:codex"]?.some(
					(message) => message.text === "Durable despite notification failure.",
				),
		).toBe(true);
