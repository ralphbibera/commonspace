...
describe("Commonspace reply-thread composer", () => {
	it("always infers Project scope for new Channel roots", async () => {
		...
		expect(
			screen.queryByRole("button", { name: "Use no Projects" }),
		).toBeNull();
		expect(
			screen.queryByRole("button", { name: "Choose Projects" }),
		).toBeNull();

		const composer = screen.getByLabelText("Post in general");
		fireEvent.change(composer, {
			target: { value: "Infer the right Projects." },
		});
	...
	it("shows routing state while an accepted message awaits inference", () => {
		...
		expect(
			screen
				.getAllByRole("status", { name: "Routing message" })
				.map((element) => element.textContent),
		).toEqual(["Routing…", "Routing…"]);
	...
	it("shows inspectable routing assignments with scoped Projects", () => {
		...
		expect(
			screen.getAllByText("@Frontend · Commonspace · inferred"),
		).toHaveLength(2);
	...
	it("reroutes one assignment with corrected Agent, wording, and Projects", async () => {
		const { rerouteAssignment } = renderChannelThread(
			"complete",
			false,
			true,
			false,
		...
		fireEvent.click(
			mustExist(
				screen.getAllByRole("button", {
					name: "Reroute assignment for Frontend",
				})[0],
			),
		);
		...
		expect(
			within(rerouteAgent).queryByRole("option", { name: "Reviewer" }),
		).toBeNull();
		fireEvent.change(rerouteAgent, { target: { value: "backend" } });
		fireEvent.change(within(form).getByLabelText("Corrected sub-request"), {
			target: { value: "Review only the UI boundary." },
		});
	...
	it("keeps superseded routing attempts visible and reroutes only the current attempt", () => {
		...
		expect(
			screen.queryByRole("button", { name: "Reroute assignment for Frontend" }),
		).toBeNull();
		expect(
			screen.getAllByRole("button", {
				name: "Reroute assignment for Reviewer",
			}),
		).toHaveLength(2);
	});
	...
	it("inspects, edits, and compacts Thread context", async () => {
		...
		fireEvent.click(
			screen.getByRole("button", { name: "Open thread context" }),
		);
		const inspector = screen.getByRole("region", { name: "Thread context" });
		expect(
			within(inspector).getByText("Inherited Channel summary."),
		).toBeTruthy();
		const form = within(inspector).getByRole("form", {
			name: "Edit Thread context",
		});
		fireEvent.change(within(form).getByLabelText("Thread summary"), {
			target: { value: "Human-focused Thread summary." },
		});
		fireEvent.change(within(form).getByLabelText("Thread decisions"), {
			target: { value: "Keep this focused.\nVerify before merge." },
		});
		fireEvent.submit(form);
		...
		fireEvent.click(
			within(inspector).getByRole("button", { name: "Compact Thread context" }),
		);
		await waitFor(() => {
	...
	it("inherits inferred Projects for Thread replies without explicit controls", async () => {
		...
		fireEvent.change(reply, {
			target: { value: "Continue with inferred context." },
		});
		fireEvent.submit(mustExist(reply.closest("form")));

		await waitFor(() => {
			expect(send).toHaveBeenCalledWith(
				"Continue with inferred context.",
				"thread-1",
				[],
			);
		});
	...
	it("adds, removes, and sources Thread pins", async () => {
		...
		fireEvent.click(
			within(inspector).getByRole("button", {
				name: "Remove pin Pinned Thread guidance.",
			}),
		);
		...
		fireEvent.change(note, {
			target: { value: "Keep this new evidence visible." },
		});
		fireEvent.submit(mustExist(note.closest("form")));
		await waitFor(() => {
		...
		fireEvent.click(
			mustExist(
				screen.getAllByRole("button", { name: "Pin message from Ralph" })[0],
			),
		);
		expect(addPin).toHaveBeenCalledWith({
			scope: { kind: "thread", id: "thread-1" },
	...
	it("edits only human messages into a new branch without explicit Project controls", async () => {
		...
		expect(
			screen.queryByRole("button", { name: "Edit message from Frontend" }),
		).toBeNull();
		fireEvent.click(
			mustExist(
				screen.getAllByRole("button", { name: "Edit message from Ralph" })[0],
			),
		);
		...
		fireEvent.change(within(form).getByLabelText("Edited message"), {
			target: { value: "Corrected investigation scope." },
		});
		fireEvent.submit(form);
	...
	it("confirms deletion of delivered content", async () => {
		...
		fireEvent.click(
			mustExist(
				screen.getAllByRole("button", { name: "Delete message from Ralph" })[0],
			),
		);
	...
	it("renders a visible marker for deleted delivered content", () => {
		...
		expect(
			screen.getAllByText(/Message deleted · content was delivered at/u),
		).toHaveLength(2);
	...
	it("navigates from an edited message to its preserved previous version", () => {
		const { selectThread } = renderChannelThread(
			"complete",
			false,
			true,
			false,
		...
		fireEvent.click(
			mustExist(
				screen.getAllByRole("button", {
					name: "Open previous message version",
				})[0],
			),
		);
	...
	it("shows only harness-advertised permission choices without blocking the Thread", async () => {
		const { respondPermission } = renderChannelThread(
			"complete",
			false,
			true,
			false,
		...
		const request = screen.getByRole("region", {
			name: "Permission request from Frontend",
		});
	...
	it("shows every agent who replied beside the thread reply count", () => {
		...
		expect(
			threadSummary.querySelector('[aria-label="Frontend replied"]'),
		).toBeTruthy();
		expect(
			threadSummary.querySelector('[aria-label="Backend replied"]'),
		).toBeTruthy();
		expect(
			threadSummary.querySelectorAll('[aria-label$="replied"]'),
		).toHaveLength(2);
	});

	it("shows a notification dot for unseen agent replies and marks them read when the thread opens", () => {
		const { mutate, selectThread } = renderChannelThread(
			"complete",
			false,
			false,
		);
		...
		const threadSummary = screen.getByRole("button", {
			name: "3 replies, 2 unread",
		});
		...
		expect(selectThread).toHaveBeenCalledWith("thread-1");
		expect(
			screen.getByRole("complementary", { name: "Thread replies" }),
		).toBeTruthy();
	...
	it("shows only the responding agents as animated icons on an active reply thread", () => {
		...
		const liveActivity = screen.getByRole("status", {
	...
	it("collapses live agent traces by default and shows one at a time when opened", () => {
		...
		expect(
			screen.queryByRole("region", { name: "Frontend live activity" }),
		).toBeNull();
		expect(
			screen.queryByRole("region", { name: "Backend live activity" }),
		).toBeNull();
		...
		expect(
			screen.getByRole("region", { name: "Frontend live activity" })
				.textContent,
		).toContain("Tracing the request through the UI.");
		...
		expect(
			screen.getByRole("region", { name: "Backend live activity" }).textContent,
		).toContain("Checking the API lifecycle.");
	});
	...
	it("offers tag autocomplete and sends the selected tag in the active thread", async () => {
		...
		await waitFor(() => {
			expect(send).toHaveBeenCalledWith("@backend", "thread-1", []);
		});
	});
	...
	it("separates agents outside the channel and explains that tagging adds them", () => {
		...
		expect(
			screen.getByText("Not in this channel · tagging adds them"),
		).toBeTruthy();
		expect(
			screen.getByRole("option", { name: /@reviewer.*will be added/i }),
		).toBeTruthy();
	});
	...
	it("replies directly to the agent selected from a channel message", async () => {
		...
		fireEvent.click(
			screen.getByRole("button", { name: "Reply directly to Frontend" }),
		);
		...
		fireEvent.change(composer, {
			target: { value: "Check that boundary again." },
		});
		fireEvent.keyDown(composer, { key: "Enter" });

		await waitFor(() => {
			expect(sendDirectReply).toHaveBeenCalledWith(
	...
	it("scrolls the thread to the bottom when the Reply button sends a message", async () => {
		const { send } = renderChannelThread();
		const composer = screen.getByRole("textbox", { name: "Reply in thread" });
		const messagesViewport = screen.getByRole("log", {
		...
		await waitFor(() => {
			expect(send).toHaveBeenCalledWith("A new thread reply.", "thread-1", []);
		});
	});
	...
	it("offers slash commands and executes them in the active thread", async () => {
		...
		await waitFor(() => {
			expect(send).toHaveBeenCalledWith("Please investigate", "thread-1");
		});
