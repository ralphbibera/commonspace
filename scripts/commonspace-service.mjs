...
import { access, chmod, lstat, mkdir, readFile, readlink, rename, rm, stat, symlink, writeFile } from 'node:fs/promises'
import { homedir } from 'node:os'
import { basename, dirname, isAbsolute, join, relative, resolve } from 'node:path'

export const COMMONSPACE_SERVICE_LABEL = 'dev.commonspace.service'
export const DEFAULT_COMMONSPACE_SOURCE = 'git@github.com:ralphbibera/commonspace.git'
...
function defaultUid() {
  ...
  if (!Number.isSafeInteger(uid) || uid < 0) throw new Error('Commonspace service installation requires a local owner UID')
  return uid
}
...
export function serviceLayout(options = {}) {
  ...
  const appRoot = resolve(options.appRoot ?? join(home, 'Library', 'Application Support', 'Commonspace'))
  if (appRoot === home || !basename(appRoot).toLocaleLowerCase().includes('commonspace')) {
    throw new Error('Commonspace installation root must be a dedicated Commonspace directory')
  }
  ...
  return {
    ...
    launchAgentPath: join(home, 'Library', 'LaunchAgents', `${COMMONSPACE_SERVICE_LABEL}.plist`),
...
export function launchAgentPlist(layout, dependencies = {}) {
  ...
  const pathEnvironment = dependencies.pathEnvironment ?? process.env.PATH ?? '/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin'
  const values = {
    nodePath,
...
async function defaultRun(command, args, options = {}) {
  return new Promise(resolveRun => {
    execFile(command, args, {
      ...
      resolveRun({
        exitCode: typeof error?.code === 'number' ? error.code : error === null || error === undefined ? 0 : 1,
        stdout,
...
async function defaultHealth(url) {
  try {
    const response = await fetch(`${url}/api/health`, { signal: globalThis.AbortSignal.timeout(1_000) })
...
function dependencies(overrides = {}) {
  return {
    ...
    pathEnvironment: overrides.pathEnvironment ?? process.env.PATH ?? '/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin',
    now: overrides.now ?? (() => new Date().toISOString()),
    randomId: overrides.randomId ?? (() => `${String(process.pid)}-${globalThis.crypto.randomUUID()}`),
    health: overrides.health ?? defaultHealth,
    log: overrides.log ?? (message => { console.info(message) }),
    run: overrides.run ?? defaultRun,
  }
}

function requireMacOwner(runtime) {
  if (runtime.platform !== 'darwin') throw new Error('Commonspace background service installation currently supports macOS')
  if (!Number.isSafeInteger(runtime.uid) || runtime.uid < 0) throw new Error('Commonspace service installation requires a local owner UID')
}
...
function assertManagedPath(layout, path) {
  ...
  if (child === '' || child.startsWith('..') || isAbsolute(child)) throw new Error('refusing to modify a path outside the Commonspace installation')
}
...
async function waitForHealth(runtime, layout) {
  ...
  throw new Error(`Commonspace service did not become healthy at ${layout.url}`)
}
...
async function writeRegistration(layout, runtime) {
  ...
  await mkdir(join(layout.home, 'Library', 'LaunchAgents'), { recursive: true })
  await mkdir(layout.logsRoot, { recursive: true, mode: 0o700 })
  await mkdir(join(layout.home, '.local', 'bin'), { recursive: true, mode: 0o700 })
  const temporaryPlist = `${layout.launchAgentPath}.${String(process.pid)}.tmp`
  try {
    await writeFile(temporaryPlist, launchAgentPlist(layout, runtime), { mode: 0o600 })
  ...
  const commandTarget = join(layout.current, 'scripts', 'commonspace-service.mjs')
  try {
    const command = await lstat(layout.binPath)
    if (!command.isSymbolicLink()) throw new Error('refusing to overwrite an unrelated Commonspace command')
    const existingTarget = resolve(dirname(layout.binPath), await readlink(layout.binPath))
    const child = relative(layout.appRoot, existingTarget)
    if (existingTarget !== commandTarget && (child.startsWith('..') || isAbsolute(child))) {
      throw new Error('refusing to overwrite an unrelated Commonspace command')
    }
    await rm(layout.binPath)
  } catch (error) {
    if (error instanceof Error && error.message === 'refusing to overwrite an unrelated Commonspace command') throw error
...
async function releaseIsComplete(path) {
  ...
  for (const file of required) {
    if (!(await stat(file)).isFile()) throw new Error(`Commonspace release is missing ${basename(file)}`)
  }
}

async function prepareRelease(layout, runtime, source) {
  await mkdir(layout.appRoot, { recursive: true, mode: 0o700 })
  await chmod(layout.appRoot, 0o700)
  const staging = join(layout.appRoot, `.staging-${runtime.randomId().replaceAll(/[^a-zA-Z0-9_.-]/gu, '-')}`)
  assertManagedPath(layout, staging)
  try {
    await requiredRun(runtime, 'git', ['clone', '--depth', '1', '--branch', 'main', '--', source, staging])
    await requiredRun(runtime, 'corepack', ['pnpm', 'install', '--frozen-lockfile'], { cwd: staging })
    ...
    const revision = (await requiredRun(runtime, 'git', ['rev-parse', 'HEAD'], { cwd: staging })).stdout.trim()
    if (!/^[a-zA-Z0-9._-]{1,200}$/u.test(revision)) throw new Error('Commonspace release returned an invalid revision')
    await writeFile(join(staging, 'commonspace-release.json'), `${JSON.stringify({ revision, source, installedAt: runtime.now() }, null, 2)}\n`, { mode: 0o600 })
    return staging
  } catch (error) {
...
export async function stopService(options = {}, overrides = {}) {
  ...
  const layout = serviceLayout({ ...options, home: runtime.home, uid: runtime.uid })
  const result = await runtime.run('/bin/launchctl', ['bootout', layout.target])
...
export async function startService(options = {}, overrides = {}) {
  ...
  requireMacOwner(runtime)
  const layout = serviceLayout({ ...options, home: runtime.home, uid: runtime.uid })
  if (!(await exists(layout.launchAgentPath)) || !(await exists(layout.current))) throw new Error('Commonspace is not installed')
  const bootstrap = await runtime.run('/bin/launchctl', ['bootstrap', layout.domain, layout.launchAgentPath])
  if (bootstrap.exitCode !== 0) {
    const loaded = await runtime.run('/bin/launchctl', ['print', layout.target])
    if (loaded.exitCode !== 0) throw new Error(`launchctl bootstrap failed: ${bootstrap.stderr.trim() || bootstrap.stdout.trim()}`)
  }
  await requiredRun(runtime, '/bin/launchctl', ['kickstart', '-k', layout.target])
...
async function activateRelease(layout, runtime, staging) {
  ...
  try {
    ...
    await stopService({ appRoot: layout.appRoot }, runtime)
    const failed = join(layout.appRoot, `.failed-${runtime.randomId().replaceAll(/[^a-zA-Z0-9_.-]/gu, '-')}`)
    ...
    runtime.log('Removed the failed release after restoring the previous installation.')
    throw error
  }
}
...
export async function installOrUpdate(options = {}, overrides = {}) {
  ...
  if (mode !== 'install' && mode !== 'update') throw new Error('service mode must be install or update')
  const layout = serviceLayout({ ...options, home: runtime.home, uid: runtime.uid })
  if (mode === 'update' && !(await exists(layout.current))) throw new Error('Commonspace is not installed; run install first')
  const source = options.source ?? DEFAULT_COMMONSPACE_SOURCE
  if (typeof source !== 'string' || source.trim() === '') throw new Error('Commonspace source is required')
  const staging = await prepareRelease(layout, runtime, source)
  await activateRelease(layout, runtime, staging)
  const metadata = JSON.parse(await readFile(layout.releaseMetadata, 'utf8'))
  runtime.log(`${mode === 'install' ? 'Installed' : 'Updated'} Commonspace ${String(metadata.revision)}.`)
...
export async function rollbackRelease(options = {}, overrides = {}) {
  ...
  requireMacOwner(runtime)
  const layout = serviceLayout({ ...options, home: runtime.home, uid: runtime.uid })
  if (!(await exists(layout.current)) || !(await exists(layout.previous))) throw new Error('no Commonspace rollback release is available')
  await stopService({ appRoot: layout.appRoot }, runtime)
  const swap = join(layout.appRoot, `.rollback-${runtime.randomId().replaceAll(/[^a-zA-Z0-9_.-]/gu, '-')}`)
...
export async function serviceStatus(options = {}, overrides = {}) {
  ...
  requireMacOwner(runtime)
  const layout = serviceLayout({ ...options, home: runtime.home, uid: runtime.uid })
  const installed = await exists(layout.current) && await exists(layout.launchAgentPath)
  const loaded = installed && (await runtime.run('/bin/launchctl', ['print', layout.target])).exitCode === 0
...
async function runCli() {
  ...
  if (command === 'start') {
    ...
    console.info(`Commonspace is healthy at ${layout.url}`)
    return
  }
  if (command === 'stop') {
    await stopService(options)
    console.info('Commonspace service stopped.')
    return
  }
  if (command === 'restart') {
    await stopService(options)
    const layout = await startService(options)
    console.info(`Commonspace restarted at ${layout.url}`)
    return
  }
  if (command === 'rollback') {
  ...
  if (command === 'status') {
    ...
    console.info(`Commonspace: ${status.healthy ? 'healthy' : status.loaded ? 'unhealthy' : status.installed ? 'stopped' : 'not installed'}`)
    console.info(`Release: ${status.release ?? 'unknown'}`)
    console.info(`URL: ${status.url}`)
    if (!status.healthy) process.exitCode = 1
    return
  }
  console.info('Usage: commonspace <install|update|start|stop|restart|status|rollback> [--source <git-url>] [--app-root <path>]')
}

const invokedPath = process.argv[1]
if (invokedPath === '-' || invokedPath !== undefined && ['commonspace', 'commonspace-service.mjs'].includes(basename(invokedPath))) {
